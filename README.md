...
<b>SEIR-T</b> is a compartmental formulation that describes SARS-Cov-2 transmission and the corresponding contact tracing in terms of network-defined neighborhoods. The original study describing SEIR-T can be consulted here: <a href="https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0259969" target="_blank">Rusu et al., 2021</a>

Our codebase also supports <b>discrete-time control events</b> generated by network-based autonomous agents. For more details, please refer to our <a href="https://github.com/andrei-rusu/control-diffusion" target="_blank">agents repository</a>, as well as the paper we published on this topic: <a href="https://link.springer.com/chapter/10.1007/978-3-031-34586-9_14" target="_blank">Rusu et al., 2022</a>.

<h2> Installation: </h2>

To explore the basic functionality of the Simulator, you can install this library via `pip`:
```
pip install .
```
The dependencies could also be installed via `conda` utilizing the environment file `conda_basic.yml` located in the `config` folder. For more options and functionality, one has the option to install with extras (please consult `setup.py` for more details).



To utilize the full <b>EpiCURB</b> framework, which includes the control agents, one should install our library via the following command, which also ensures that the `control_diffusion` library is available:
```
pip install .[control_learn]
```
Again, the dependencies could be installed via `conda` instead, relying on the environment file `conda_jupyter_control.yml`. <b> Important note:</b> Our GNN-based control agents require both `torch_scatter` and `torch_sparse` to be installed via the wheel compatible with the PyTorch and CUDA versions available in the programming environment. If neither `pip` nor `conda` managed to install the correct versions, please install them manually by following the <a href="https://pytorch-geometric.readthedocs.io/en/latest/install/installation.html" target="_blank">official documentation</a>.
