...
def process_json_results(path=None, print_id_fail=True, keys=('pa', 'dual'), overlap_to_capture='overlap'):
    """
    ...
    for file in glob.glob(path):
        try:
            ...
            taur = args['taur']
            over = float(round(args.get(overlap_to_capture, 1.), 2))
            uptake = float(round(args.get('uptake', 1.), 2))
            key_vals = [args[k] for k in keys]
            ...
            for taut_entry in taut:
                ...
                # allow for the parameter keys to dictate the structure of the nested dictionary
                entry = all_sim_res
                for kv in key_vals:
                    entry = entry[kv]
                entry[uptake][over][taut_entry][taur] = results
