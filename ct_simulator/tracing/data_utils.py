...
def load_socialevol(agg='D', proxim_prob=None):
    """
    Loads the SocialEvolution dataset from the specified file paths and returns filtered and aggregated data.

    Args:
        agg (str): The time aggregation level to use. Can be 'D' for daily or 'W' for weekly. Defaults to 'D'.
        proxim_prob (float): The proximity probability threshold to use for filtering proximity data. Defaults to None.

    Returns:
        A tuple containing two pandas dataframes:
        - The filtered and aggregated proximity data.
        - The filtered and aggregated SMS and call data, merged and summed over based on week, source, and destination.
    """
def load_socialevol(agg='D', proxim_prob=None):
    ...
    def get_counts_by_time_agg(df):
        # convert uids to int and date to datetime64[D]
...
class DataLoader():
    ...
    def get_edge_data_for_time(self, which_inf=0, which_tr=1, time_fr='2009-01-05', time_to='2009-06-14', use_week_index=True):
        """
        time_fr/to can either be strings marking dates or integers marking the time ids (day or week ids) directly
        """
        ...
        if which_tr is not None and which_tr >= 0:
            ...
            tr = tr.loc[tr.uid.isin(nodes) & tr.dst.isin(nodes) & tr.time.between(time_fr, time_to)]
